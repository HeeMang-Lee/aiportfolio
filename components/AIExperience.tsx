...
const timelineItems = [
  {
    ...
    whyAI:
      "테스트 코드 작성은 AI에게 위임하기 적합한 작업이라 판단했다. 다만 처음 맡겼을 때 파라미터 검증, null 체크 같은 의미 없는 테스트가 과도하게 생성되는 문제가 있었다. 검수 과정에서 이를 걸러내고, \"잔고 부족 시 주문 실패\", \"중복 종목 포트폴리오 생성\" 등 실제 비즈니스 로직을 검증하는 시나리오를 명시하여 방향을 수정했다.",
    result:
  ...
  },
  {
    ...
    result:
      "락 대기시간 60분 → 0.15초(24,000배 개선), 캐시 히트율 70.8%, N+1 쿼리 1,001회 → 2회. 학습한 지식을 AI와 함께 프로젝트에 맞게 적용하고, 실측 데이터 기반으로 의사결정한 경험이다.",
  ...
  },
  {
    ...
    result:
      "반복 설명 없이 일관된 결과물을 얻을 수 있게 됐다. 이 포트폴리오 사이트 자체가 이 방식의 결과물이다. 프론트엔드 경험 없이 Claude Code만으로 제작했다.",
...
const concreteUseCases = [
  {
    ...
    detail:
      "AI에게 테스트 작성을 맡겼을 때 파라미터 타입 체크, null 검증 같은 의미 없는 테스트가 대량 생성됐다. 검수에서 이를 걸러내고, \"주문 실행 시 잔고 부족\", \"포트폴리오 생성 시 중복 종목 처리\" 등 실제 비즈니스 로직을 검증하는 시나리오를 구체적으로 명시하여 재지시했다. AI는 코드를 생성하고, 무엇을 테스트해야 하는지는 내가 판단했다.",
  },
  {
    emoji: "🏗️",
