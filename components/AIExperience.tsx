...
const timelineItems = [
  ...
  {
    period: "2026.03 - 현재",
    title: "AI 에이전트 역할 분리 + 컨텍스트 시스템화",
    situation:
      "하나의 AI 에이전트에 모든 작업을 맡기면 대화가 길어질수록 맥락이 흐려졌다. 에이전트가 재시작될 때마다 아키텍처 규칙, 네이밍 컨벤션, 금지사항을 처음부터 다시 설명해야 하는 비효율이 반복됐다. AI의 성능 차이는 도구 자체보다 사용 방법의 편차에서 온다고 판단했다.",
    whyAI:
      "개인의 프롬프트 역량에 의존하지 않고, 시스템으로 품질의 저점을 방어하는 방식을 택했다. 아키텍처 원칙, 패키지 구조, 구현 순서, 금지사항(@Setter 금지, 순환 의존 금지 등)을 skills.md에 문서화하여 누가 에이전트를 실행해도 동일한 컨텍스트에서 시작하도록 설계했다. 팀 협업 시에도 이 지시서를 공유하면 AI 활용 규칙을 통일할 수 있다.",
    result:
      "반복 설명 없이 일관된 결과물을 얻을 수 있게 됐다. 현재 개인 블로그 프로젝트에 적용 중이며, 에이전트가 교체되어도 지시서 기반으로 동일한 품질 기준을 유지하고 있다. 1명이 정리한 지식이 이후 모든 AI 세션의 컨텍스트가 되는 축적형 구조다.",
    tags: ["에이전트분리", "컨텍스트시스템화", "현재진행형"],
    current: true,
  },
];
...
const concreteUseCases = [
  ...
  {
    emoji: "📋",
    title: "개인의 프롬프트가 아닌 시스템으로 품질을 방어",
    context: "블로그 프로젝트 | 컨텍스트 시스템화",
    detail:
      "아키텍처 원칙(실용적 헥사고날), 패키지 구조, 네이밍 컨벤션, 구현 순서, 금지사항을 skills.md에 문서화했다. 누가 에이전트를 실행해도 동일한 컨텍스트에서 시작하고, 발견한 규칙은 지시서에 축적된다. 대화와 함께 소멸하는 지식이 아니라, Git에 남아 팀 전체의 컨텍스트가 되는 구조다.",
  },
];
