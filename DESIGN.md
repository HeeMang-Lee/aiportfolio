...
**이동과 체류와 정지를 번갈아 둔다.** 판이 화면 가운데로 들어오면 거기서 멈춰 서고(체류), 그 동안 스크롤이 그 프로젝트의 트러블슈팅을 위로 흘려보낸다. 다 흐르고 나면 그 자리에서 잠깐 멈춘다(정지). 그 다음 판으로 넘어간다(이동).

**정지 구간이 없으면 마지막 상세는 한 번도 읽히지 않는다.** 상세 기둥의 이동 거리는 `기둥 높이 - 창 높이` 인데, 마지막 블록의 바닥이 곧 기둥의 바닥이라 그 블록은 체류가 끝나는 순간에야 제자리에 온다. 그리고 바로 다음 판으로 밀려난다. 정지 `1.2` 단위가 읽을 자리다.

**체류 길이는 글 길이가 정한다.** 넘치는 픽셀만큼 스크롤을 쓰게 해서 일반 페이지를 읽는 것과 같은 속도(1:1)로 만든다. 고정값으로 두면 긴 글은 날아가고 짧은 글은 늘어진다. 이동 `0.8`, 정지 `1.2` 단위이고 1단위는 `0.7vh` 다.
